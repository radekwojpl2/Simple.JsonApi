...
- [ ] T001 Create `libs/tests/JsonApiLite.OpenApi.Tests/JsonApiLite.OpenApi.Tests.csproj` targeting
...
- [ ] T002 Add `InternalsVisibleTo("JsonApiLite.OpenApi.Tests")` to
...
- [ ] T003 Register the new project in `JsonApiLite.sln` so `dotnet test JsonApiLite.sln -c Release`
...
- [ ] T004 [P] Create fixture contracts in `libs/tests/JsonApiLite.OpenApi.Tests/TestContracts.cs`:
...
- [ ] T005 [P] Create `libs/tests/JsonApiLite.OpenApi.Tests/SchemaFixture.cs` — a helper that builds
...
- [ ] T006 Add `libs/tests/JsonApiLite.OpenApi.Tests/EnvelopeBaselineTests.cs` pinning **today's**
...
- [ ] T007 Replace the single-line envelope in `libs/JsonApiLite.OpenApi/JsonApiSchemaBuilder.cs:32`
...
- [ ] T008 Gate envelope members on `body is JsonApiResponseBody` in
...
- [ ] T009 [P] Add `libs/tests/JsonApiLite.OpenApi.Tests/RequestSchemaTests.cs` asserting a request
...
- [ ] T010 [P] Add `libs/tests/JsonApiLite.OpenApi.Tests/RequiredMembersTests.cs` asserting that
...
- [ ] T011 [P] [US1] Add `libs/tests/JsonApiLite.OpenApi.Tests/DocumentResolutionTests.cs` asserting
...
- [ ] T012 [P] [US1] In the same file, assert the emitted `data` for `ResourceDocument<A,R,M,I>` is
      **identical** to that for `ResourceDocument<A,R>` — FR-002, "described identically". Compare
      serialized JSON, not object references.
- [ ] T013 [P] [US1] In the same file, assert every arity accepted today is still accepted:
...
- [ ] T014 [P] [US1] In the same file, assert an unsupported type (e.g. `string`) still throws
...
- [ ] T015 [US1] Rework `Describe(Type)` in `libs/JsonApiLite.OpenApi/JsonApiBody.cs:62-102` to
...
- [ ] T016 [US1] Extend `JsonApiBody.Description` (`libs/JsonApiLite.OpenApi/JsonApiBody.cs:118-123`)
...
- [ ] T017 [US1] Update the XML doc on `JsonApiBody` (`JsonApiBody.cs:18-21`) to describe resolution
...
- [ ] T018 [P] [US2] Add `libs/tests/JsonApiLite.OpenApi.Tests/MetaSchemaTests.cs` asserting a
      declared meta record is walked into named, typed members (FR-004).
- [ ] T019 [P] [US2] In the same file, assert a document whose `TMeta` is `Meta` emits
...
- [ ] T020 [P] [US2] In the same file, assert the same for `Meta<TMeta>`, which satisfies the
...
- [ ] T021 [P] [US2] In the same file, assert a declared shape containing a nested object, a list and
...
- [ ] T022 [US2] In `libs/JsonApiLite.OpenApi/JsonApiBody.cs`, normalise `Description.Meta` to `null`
...
- [ ] T023 [US2] In `libs/JsonApiLite.OpenApi/JsonApiSchemaBuilder.cs`, emit the `meta` member on
...
- [ ] T024 [P] [US3] Add `libs/tests/JsonApiLite.OpenApi.Tests/LinksSchemaTests.cs` asserting the
...
- [ ] T025 [P] [US3] In the same file, assert each link member is an `anyOf` of a string with
...
- [ ] T026 [P] [US3] In the same file, assert **no** emitted document anywhere contains a
...
- [ ] T027 [P] [US3] In the same file, assert pagination links appear **only** on collection-primary
      kinds, per the spec: "Pagination links **MUST** appear in the links object that corresponds to a
      collection."

### Implementation for User Story 3

- [ ] T028 [US3] Add a hand-written `Links(JsonApiShape, bool collection)` builder to
...
- [ ] T029 [US3] Emit the `links` member on response envelopes from the seam added in T007, and
...
- [ ] T030 [P] [US4] Add `libs/tests/JsonApiLite.OpenApi.Tests/IncludedSchemaTests.cs` asserting a
...
- [ ] T031 [P] [US4] In the same file, assert a document resolving to `AnyIncluded` describes an
...
- [ ] T032 [P] [US4] In the same file, assert linkage and error documents never describe `included`
      (FR-022), and that a sideloadable type equal to the primary data's type does not collide with
      the primary data's own schema.
- [ ] T033 [P] [US4] Add `libs/tests/JsonApiLite.Tests/Serialization/IncludedDeclarationTests.cs`
...
- [ ] T034 [US4] Add the public accessor to the core package in
...
- [ ] T035 [US4] In `libs/JsonApiLite.OpenApi/JsonApiBody.cs`, normalise `Description.Included` to
      `null` when the fourth type argument is `AnyIncluded`, which declares nothing
      (`AnyIncluded.cs:7-8`).
- [ ] T036 [US4] In `libs/JsonApiLite.OpenApi/JsonApiSchemaBuilder.cs`, emit `included` on resource
...
- [ ] T037 Verify against the sample. Temporarily replace the two `PackageReference` lines in
...
- [ ] T038 Walk [quickstart.md](quickstart.md) end to end against the captured document, confirming
      every story's expected output and both contrast cases (unconstrained `meta`, unconstrained
      `included`).
- [ ] T039 Validate a real sample response against the published description (FR-017, SC-005).
...
- [ ] T040 Run the full constitution gate and report the output verbatim:
...
- [ ] T041 [P] Update `README.md` to document the envelope members now described and the new core
      accessor, and check `ROADMAP.md` for an entry to mark advanced.
- [ ] T042 Confirm the core package still has zero package references and both TFMs build
